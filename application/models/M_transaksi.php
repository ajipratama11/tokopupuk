...
class M_transaksi extends CI_Model{
	...
	function tampil_pesan12($idpesan){
		$query = $this->db->query("SELECT * FROM `pemesanan` JOIN konfirmasi_pemesanan ON pemesanan.id_trans=konfirmasi_pemesanan.id_trans JOIN customer ON pemesanan.id_cus=customer.id_cus JOIN barang ON pemesanan.id_barang=barang.id_barang WHERE pemesanan.id_pemesanan='$idpesan'");
		return $query->result();
	}
